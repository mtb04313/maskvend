...
#if (FEATURE_TRUST_M == DISABLE_FEATURE)
// CapSense Tuner uses the same I2C pins as Trust-M
...
static void capsense_timer_callback(cy_timer_callback_arg_t xTimer);
...
* Global variables
******************************************************************************/
cy_thread_t g_capsense_task_handle = NULL;
cy_queue_t g_capsense_command_q = NULL;
cy_timer_t scan_timer_handle = NULL;
...
static const char *TAG = "capsense_task";
...
*
*******************************************************************************/
void task_capsense(cy_thread_arg_t param)
{
    cy_status status;
    ...
    if (CY_RSLT_SUCCESS != cy_rtos_init_timer(&scan_timer_handle,
                                              CY_TIMER_TYPE_PERIODIC,
                                              capsense_timer_callback,
                                              (cy_timer_callback_arg_t)scan_timer_handle)) {
        CY_LOGD(TAG, "cy_rtos_init_timer(scan_timer_handle) failed!");
        DEBUG_ASSERT(0);
    }
    ...
    /* Start the timer */
    if (CY_RSLT_SUCCESS != cy_rtos_start_timer(&scan_timer_handle,
                                               CAPSENSE_SCAN_INTERVAL_MS)) {
        CY_LOGD(TAG, "cy_rtos_start_timer(scan_timer_handle) failed!");
        DEBUG_ASSERT(0);
    }
    ...
    for(;;)
    {
        ...
        if (CY_RSLT_SUCCESS == cy_rtos_get_queue(   &g_capsense_command_q,
                                                    (void *)&capsense_cmd,
...
static void process_touch(void)
{
    ...
    /* Variables used for storing command and data for GUI Mask Task */
    gui_mask_command_data_t gui_mask_cmd_data;
    bool send_gui_mask_command = false;
    ...
    if((0u != button0_status) && (0u == button0_status_prev))
    {
        ...
        gui_mask_cmd_data.command = GUI_CAPSENSE_OK;
        send_gui_mask_command = true;
    }
    ...
    if((0u != button1_status) && (0u == button1_status_prev))
    {
        ...
        gui_mask_cmd_data.command = GUI_CAPSENSE_CANCEL;
        send_gui_mask_command = true;
    }
    ...
    if((0u != slider_touched) && (slider_pos_perv != slider_pos ))
    {
        ...
        gui_mask_cmd_data.command = GUI_CAPSENSE_UPDATE_POSITION;

        /* position in percentage */
        gui_mask_cmd_data.position = (slider_pos * 100)
                    / cy_capsense_context.ptrWdConfig[CY_CAPSENSE_LINEARSLIDER0_WDGT_ID].xResolution;
        send_gui_mask_command = true;
    }
    ...
    if(send_led_command) {
        if (CY_RSLT_SUCCESS != cy_rtos_put_queue(   &g_led_command_data_q,
                                                    (void *)&led_cmd_data,
                                                    0,
                                                    false
                                                )) {
            CY_LOGD(TAG, "cy_rtos_put_queue(g_led_command_data_q) failed!");
        }
    }

    /* Send command to update GUI Mask state if required */
    if(send_gui_mask_command) {
        if (CY_RSLT_SUCCESS != cy_rtos_put_queue(   &g_gui_mask_command_data_q,
                                                    (void *)&gui_mask_cmd_data,
                                                    0,
                                                    false
                                                )) {
            // gets too verbose when slider is used
...
static void capsense_end_of_scan_callback(cy_stc_active_scan_sns_t* active_scan_sns_ptr)
{
    ...
    if (CY_RSLT_SUCCESS != cy_rtos_put_queue(   &g_capsense_command_q,
                                                (void *)&commmand,
                                                0,
                                                true
                                            )) {
